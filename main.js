...
function handleSubmit() {
	...
	lastPayload = payload;   // ✅ ADD THIS LINE HERE
	...
	// ✅ Add biometric files (VERY IMPORTANT)
	// Example: Face
	if (state.face.front && state.face.front.file) {
		formData.append("faceFront", state.face.front.file);
	}

	if (state.face.left && state.face.left.file) {
		formData.append("faceLeft", state.face.left.file);
	}

	if (state.face.right && state.face.right.file) {
		formData.append("faceRight", state.face.right.file);
	}

	// ✅ Fingerprints
	["left", "right"].forEach(hand => {
		["thumb", "index", "middle", "ring", "little"].forEach(finger => {
			const data = state.fingerprints[hand][finger];
			if (data && data.file) {
				formData.append(`fp_${hand}_${finger}`, data.file);
			}
		});
	});

	// ✅ Iris
	if (state.iris.left && state.iris.left.file) {
		formData.append("irisLeft", state.iris.left.file);
	}

	if (state.iris.right && state.iris.right.file) {
		formData.append("irisRight", state.iris.right.file);
	}
	...
	fetch("enrollment", {
		method: "POST",
		body: formData   // ❗ NO Content-Type header (important)
	})
		.then(data => {
			console.log("✅ Response:", data);

			lastResponse = data; // ✅ IMPORTANT

			// safe toast (if not defined)
			if (typeof showToast === "function") {
				if (data.status === "success") {
					showToast(data.message || "Enrollment successful!");
				} else {
					showToast(data.message || "Enrollment failed", true);
				}
			} else {
				console.log(data.message);
			}

			// ✅ FIXED: pass response also
			displayPayload(lastPayload, data);
		})
		.catch(err => {
...
/*
	function displayPayload(payload, backendResponse) {
		// ✅ Render request JSON
		const jsonStr = JSON.stringify(payload, null, 2);
		...
		if (jsonBlock) {
			if (typeof syntaxHighlight === "function") {
				jsonBlock.innerHTML = syntaxHighlight(jsonStr);
			} else {
				jsonBlock.innerText = jsonStr; // fallback
			}
		}
		...
		// ✅ Render backend response safely
		const respBlock = document.getElementById('responseBlock');
		const respSection = document.getElementById('responseSection');

		if (respBlock) {
			if (backendResponse) {
				const respStr = JSON.stringify(backendResponse, null, 2);

				if (typeof syntaxHighlight === "function") {
					respBlock.innerHTML = syntaxHighlight(respStr);
				} else {
					respBlock.innerText = respStr;
				}

				// optional styling
				if (respSection) {
					const isSuccess = backendResponse.status === 'success';
					respSection.style.borderColor = isSuccess ? 'green' : 'red';

					const waitingEl = respSection.querySelector('.response-waiting');
					if (waitingEl) waitingEl.style.display = 'none';
				}

			} else {
				respBlock.innerText = "Waiting for response...";
			}
		}
		...
		const overlay = document.getElementById('payloadOverlay');
		if (overlay) {
			overlay.style.display = 'block'; // safer than classList
		}
...
function displayPayload(payload, backendResponse) {
	...
	// ── DEBUG: add these 4 lines temporarily ──
	   console.log("backendResponse received:", backendResponse);
	   const respBlock2 = document.getElementById('responseBlock');
	   const respSection2 = document.getElementById('responseSection');
	   console.log("respBlock found:", !!respBlock2);
	   console.log("respSection found:", !!respSection2);
	   console.log("waitingEl found:", !!respSection2?.querySelector('.response-waiting'));
	   console.log("respSection innerHTML:", respSection2?.innerHTML?.substring(0, 200));
	   // ── END DEBUG ──
    ...
    const jsonBlock = document.getElementById('jsonBlock');
    if (jsonBlock) {
        jsonBlock.innerHTML = typeof syntaxHighlight === "function"
            ? syntaxHighlight(jsonStr)
            : jsonBlock.innerText = jsonStr;
    }

    if (typeof buildSummary === "function") buildSummary(payload);

    // Render backend response
  //  const respBlock = document.getElementById('responseBlock');
   // const respSection = document.getElementById('responseSection');
   
   const respBlock = document.getElementById('responseCardBody');
   const respSection = respBlock ? respBlock.closest('.response-section') : null;

    if (respBlock) {
        if (backendResponse) {
            const respStr = JSON.stringify(backendResponse, null, 2);
            respBlock.innerHTML = typeof syntaxHighlight === "function"
                ? syntaxHighlight(respStr)
                : respStr;

            if (respSection) {
                const isSuccess = backendResponse.status === 'success';
                respSection.style.borderColor = isSuccess
                    ? 'var(--success, #22c55e)'
                    : 'var(--danger, #ef4444)';
                //const waitingEl = respSection.querySelector('.response-waiting');
               // if (waitingEl) waitingEl.style.display = 'none';
			   const waitingEl = respSection ? respSection.querySelector('.response-waiting, .waiting, p, .spinner-wrap') : null;
			   if (waitingEl) waitingEl.style.display = 'none';
            }
        } else {
            respBlock.innerText = "Waiting for response...";
    ...
    // ✅ FIX: Use classList like the rest of your code expects
    const overlay = document.getElementById('payloadOverlay');
    if (overlay) {
        overlay.classList.remove('closing');
        overlay.classList.add('visible'); // ← was broken before
    }
    document.body.style.overflow = 'hidden';
}
   ...
   // add this at top of file near lastPayload
